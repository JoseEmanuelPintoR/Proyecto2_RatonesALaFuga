...
namespace Ratones.Basic
{
    ...
    {
        public const string Version = "RF2";
        ...
        public static string Snapshot(State state)
        {
            var s = new StringBuilder(7000);
            ...
            foreach (Player p in state.Players)
                s.Append(p.Id).Append(',').Append(p.Slot).Append(',').Append(p.KeyColor).Append(',').Append(p.AuraColor).Append(',')
                    ...
                    .Append(Num(p.BoostLeft)).Append(',').Append(Num(p.FreezeLeft)).Append(';');
            s.Append('|');
        ...
        {
            string[] a = line.Split('|');
            if (a.Length != 10 || a[0] != "S") throw new FormatException("Estado incompleto.");
            ...
            var state = new State { Phase = (Phase)phase, Round = int.Parse(a[2]), HostId = int.Parse(a[3]),
                TimeLeft = Float(a[4]), CountdownLeft = Float(a[5]), Result = Decode(a[6]) };
            foreach (string id in a[7].Split(',')) if (id.Length > 0) state.Winners.Add(int.Parse(id));
            foreach (string row in a[8].Split(';'))
            {
                if (row.Length == 0) continue;
                string[] p = row.Split(',');
                if (p.Length != 14 || state.Players.Count >= 4) throw new FormatException("Jugador incorrecto.");
                int slot = int.Parse(p[1]), inv = int.Parse(p[11]);
                if (slot < 0 || slot > 3 || inv < 0 || inv > 2) throw new FormatException("Estado de jugador incorrecto.");
                state.Players.Add(new Player { Id = int.Parse(p[0]), Slot = slot, KeyColor = Simulation.ColorIndex(int.Parse(p[2])),
                    ...
                    Collected = int.Parse(p[10]), Inventory = (Power)inv, BoostLeft = Float(p[12]), FreezeLeft = Float(p[13]) });
            }
