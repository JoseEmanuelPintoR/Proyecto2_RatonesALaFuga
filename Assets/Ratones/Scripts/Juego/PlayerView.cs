...
namespace Ratones.Basic
{
    public sealed class PlayerView : MonoBehaviour
    {
        ...
        {
            var target = new Vector3(player.Position.X, 0, player.Position.Z);
            bool moving = Vector3.Distance(transform.position, target) > .07f;
            transform.position = first ? target : Vector3.Lerp(transform.position, target, 1 - Mathf.Exp(-25 * Time.unscaledDeltaTime));
